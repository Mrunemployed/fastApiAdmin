...
passContext = CryptContext(schemes=["argon2"],deprecated="auto")
...
async def _create_jwt_token(data:dict,expiry:timedelta=None):
    ...
    return jwt.encode(to_encode, SECRET, algorithm=HASHING_ALGORITHM)
...
async def decode_access_token(token:str) -> dict|None:
    try:
        ...
        raise HTTPException(status_code=403, detail={"errors":"Token Expired", "message":"Your session has expired please login again."})
